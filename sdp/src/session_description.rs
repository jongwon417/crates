...
impl SessionDescription {
    ...
    /// SDP를 받는 경우 기존의 함수에서 해석하지 못하는 경우 발생
    /// s1 ~ s16의 데이터 순서가 보장되지 않음
    /// 데이터 순서가 보장되지 않아도 되도록 수정
    pub fn unmarshal<R: io::BufRead + io::Seek>(reader: &mut R) -> Result<Self, Error> {
        let mut lexer = Lexer {
            desc: SessionDescription {
                version: 0,
        ...
        let mut state = Some(StateFn { f: session_parse });
        while let Some(s) = state {
            state = (s.f)(&mut lexer)?;
        }
...
/// 세션의 필수적인 정보 v, o, s 를 먼저 파싱한다
fn session_parse<'a, R: io::BufRead + io::Seek>(
    lexer: &mut Lexer<'a, R>,
    ...
    match key.as_str() {
        ...
        _ => return session_parse_end(&key),
    }
}

/// 세션의 필수적인 정보(t), 옵션 정보를 파싱한다.
/// 적어도 t는 있어야만 하기에 key의 validation체크는 하지 않는다.
fn session_parse_end<'a, R: io::BufRead + io::Seek>(
    key: &str,
) -> Result<Option<StateFn<'a, R>>, Error> {
    match key {
        "i=" => Ok(Some(StateFn {
        ...
        _ => Err(Error::SdpInvalidSyntax(key.to_string())),
    }
}

/// 세션의 필수적인 정보(t), 옵션 정보를 파싱한다.
fn session_option_parse<'a, R: io::BufRead + io::Seek>(
    lexer: &mut Lexer<'a, R>,
) -> Result<Option<StateFn<'a, R>>, Error> {
    ...
    match key.as_str() {
        ...
        _ => session_option_parse_end(&key),
    }
}

/// 미디어의 필수적인 정보 m 를 먼저 파싱한다
fn session_option_parse_end<'a, R: io::BufRead + io::Seek>(
    key: &str
) -> Result<Option<StateFn<'a, R>>, Error> {
    match key {
        "m=" => {
            Ok(Some(StateFn { f: unmarshal_media_description }))
        }
        _ => Err(Error::SdpInvalidSyntax(key.to_string())),
    }
}
...
/// 미디어의 옵션정보를 파싱한다
fn media_option_parse<'a, R: io::BufRead + io::Seek>(
    lexer: &mut Lexer<'a, R>,
) -> Result<Option<StateFn<'a, R>>, Error> {
    let (key, num_bytes) = read_type(lexer.reader)?;
    if key.is_empty() && num_bytes == 0 {
    ...
    match key.as_str() {
        "i=" => Ok(Some(StateFn { f: unmarshal_media_title })),
        "c=" => Ok(Some(StateFn { f: unmarshal_media_connection_information })),
        "b=" => Ok(Some(StateFn { f: unmarshal_media_bandwidth })),
        "k=" => Ok(Some(StateFn { f: unmarshal_media_encryption_key })),
        "a=" => Ok(Some(StateFn { f: unmarshal_media_attribute })),
        _ => session_option_parse_end(&key),
    }
}
...
fn unmarshal_protocol_version<'a, R: io::BufRead + io::Seek>(
    ...
    Ok(Some(StateFn { f: session_parse }))
}
...
fn unmarshal_origin<'a, R: io::BufRead + io::Seek>(
    ...
    let mut i = index_of(fields[4], &["IP4", "IP6"]);
    if i == -1 {
        // 중간에 V가 들어간 프로토콜버전 이름을 사용하는 경우가 있음
        i = index_of(fields[4], &["IPV4", "IPV6"]);
    }
...
fn unmarshal_media_description<'a, R: io::BufRead + io::Seek>(
    ...
    Ok(Some(StateFn { f: media_option_parse }))
}
...
fn unmarshal_connection_information(value: &str) -> Result<Option<ConnectionInformation>, Error> {
    ...
    // Set according to currently registered with IANA
    // https://tools.ietf.org/html/rfc4566#section-8.2.7
    let mut i = index_of(fields[1], &["IP4", "IP6"]);
    if i == -1 {
        i = index_of(fields[1], &["IPV4", "IPV6"]);
    }
